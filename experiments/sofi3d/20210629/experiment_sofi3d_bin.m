...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.io.imagePath = 'data\sofi3d\test_main_bin\20210615\input';
settings.io.imageName = []; % if empty, all the files in the path will be processed
settings.io.pnc = 'data\sofi3d\test_main_bin\20210615\input\calibration'; % calibration
% input file format - the same for all image inputs
% '.tif'; '.tiff' ; '.bin' ; '.dat'
settings.io.fext = '.bin';
...
settings.sys.subseqlength = 200; % length of a subsequence used for cumulant calculation
...
%--- SOFI 3D ---%
settings.sys.subseqstep = 200; % size of a sliding window, if subseqstep = subseqlength => no overlap, if subseqstep < subseqlength => overlap of subsequences
...
settings.dec.fwhm = [3 3 2]; % sofi3d
...
% test outputpath
settings.io.outputpath = 'data\sofi3d\test_main_bin\20210615\result';
