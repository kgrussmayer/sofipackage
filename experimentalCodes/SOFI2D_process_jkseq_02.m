...
function [sofi,sofi_c,settings,snrs,vars,bias,snrs2,vars2] = SOFI2D_process_jkseq_02(settings) 
...
%         disp(fwhm); 
    ...
    if settings.sys.jk == 1
        ...
        for jj = settings.sys.start:settings.sys.wsize:floor((frames-settings.sys.start+1)/settings.sys.wsize)*settings.sys.wsize
            ...
            tic
                [c1,c2,c3]=sofiCumulantsSNRseq2(substack,1,[],[],settings.sys.orders,[],cellfun(@(x)x(:,:,[1:count-1,count+1:end]),...
...
%                     if ii <= count % do not add values from later subsequences
                    ...
                    sofi3{io,ii}=sofi3{io,ii} + c3{io,ii};
...
%         snrs=sofi1;                             % signal-to-noise ratios
        ...
        for io=settings.sys.orders
            for ii = 1:nss
               ...
               sofi3{io,ii}=sofi3{io,ii}/frame;

                    if settings.sys.block > 0
                    blockSize = repmat(ceil((blockx*104/4)/(104/io)),1,2);
                    sofi{io,ii} = blkproc(sofi{io,ii},blockSize,@(x)sum(x(:)));
                    sofi3{io,ii} = blkproc(sofi3{io,ii},blockSize,@(x)sum(x(:)));
                    end
                    ...
                    vars2{io,ii}=(frame-1)*(sofi3{io,ii} - sofi1{io,ii}.^2);
                    snrs2{io,ii}=abs(sofi{io,ii})./sqrt(vars2{io,ii});
            end
        end
