...
function [sofi,sofi_c,settings,snrs,vars,bias] = SOFI2D_process_jkseq_05(settings) 
    ...
    if isempty(settings.sys.sub)
        ...
        frames = settings.sys.sub;
    end
    
    stack = load_tifFile([settings.io.imageFile,'.tif'],[frames],settings.io.roi);
...
%     drift = drift_corr';
    ...
    for k=1:kmax %frames-1%size(drift,1) %frames-1%
        disp(['dcor: ',num2str(k)]);
        img = stack(:,:,k+1);
...
%     frames = size(stack,3);
%     blockx = 2;
%     blockSize = repmat(ceil((blockx*104/4)/(104/1)),1,2);
%     stack = stack(1:2:end-1,1:2:end-1,:)+stack(2:2:end,2:2:end,:)+stack(1:2:end-1,2:2:end,:)+stack(2:2:end,1:2:end-1,:);
    
%     for ii = 1:size(stack,3)
%     stack(:,:,ii) = blkproc(stack(:,:,ii),blockSize,@(x)sum(x(:)));
%     disp(['blockproc ',num2str(ii)])
%     end
...
%         [cx,fwhm]=sofiFlatten([],c,grid,settings.sys.orders);
%         fwhms(count) = fwhm;
%         disp(fwhm); 
...
%     
%     num = 5; % number of points used for the analysis
%     [gcor,rho_estB,coefs,parmap] = kest_v6(sofi_c,num,wfbcg,stack(:,:,1:1000));
% 
% 
%     gammas = mean(gcor,2)./(1:length(settings.sys.orders))';
%     gammas(1,:)=1;
%     settings.dec.gammas = gammas;
    %%%
    
    if settings.sys.jk == 1
        blockx = settings.sys.block ;
        ...
        for jj = settings.sys.start:settings.sys.wsize:floor((frames-settings.sys.start+1)/settings.sys.wsize)*settings.sys.wsize
            ...
            tic
                [c1,c2]=sofiCumulantsSNRseq5(substack,1,[],[],settings.sys.orders,[],cellfun(@(x)x(:,:,[1:count-1,count+1:end]),...
                    sofi_c,'UniformOutput',0),nss,count,settings.sys.block,settings.dec.gammas);
            toc
...
%                temp = cellfun(@(x)sum(x(:,:,1:ii),3),sofi_c,'UniformOutput',0);
               ...
               sofi2{io,ii}=sofi2{io,ii}/frame;
                    ...
                    if settings.sys.block > 0
                        blockSize = blockx*io;%ceil((blockx*104)/(104/io));
%                     temp = blkproc(temp,blockSize,@(x)sum(x(:)));
%                         temp = binStack(sum(abs(sofi_c{io}(:,:,1:ii)).^(gammas(io)),3), blockSize);
%                         temp = sum(binStack(abs(sofi_c{io}(:,:,1:ii)), blockSize),3).^(gammas(io));
                       
%                         temp = binStack(sum(abs(sofi_c{io}(:,:,1:ii)).^(gammas(io)),3), blockSize);
%                         temp = binStack(sum(abs(sofi_c{io}(:,:,1:ii)),3), blockSize).^(gammas(io));

%                         temp = sum(binStack(abs(sofi_c{io}(:,:,1:ii)), blockSize).^(gammas(io)),3);
                      temp = sum(binStack(abs(sofi_c{io}(:,:,1:ii)), blockSize),3).^(gammas(io));
%                     temp = mean(binStack(abs(sofi_c{io}(:,:,1:ii)), blockSize).^(gammas(io)),3);    
%                     temp = sum(temp(:,:,1:ii).^(gammas(io)),3);%./ii;
                        
%                     sofi1{io,ii} = binStack(sofi1{io,ii},blockSize);
%                     sofi2{io,ii} = binStack(sofi2{io,ii},blockSize);
                    else
                        temp = sum(abs(sofi_c{io}(:,:,1:ii)),3).^(gammas(io));%./ii;
                    end
                   
%                     temp = temp./max(temp(:));
%                       bcg1 = median(temp(:));
%                     temp = imadjust(temp,[0.8*bcg1 1],[0 1]);
%                       temp = max(temp,0.8*bcg1);
...
%                     end
                    ...
                    bias{io,ii,count}=(frame-1)*(sofi1{io,ii} - temp);
%                     vars{io,ii} = (frame-1)/frame * temp; 
%                     snrs{io,ii}=abs(sofi{io,ii})./sqrt(vars{io,ii});
                    
                    vars{io,ii}=(frame-1)*((sofi2{io,ii}) - (sofi1{io,ii}).^2);
                    snrs{io,ii}=abs(temp)./sqrt(vars{io,ii});
            end
        end
