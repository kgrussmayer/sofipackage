...
function [sofi,sofi_c,settings,snrs,vars,bias] = SOFI2D_process_jkseq_03(settings) 
...
%         disp(fwhm); 
    ...
    if settings.sys.jk == 1
        ...
        for jj = settings.sys.start:settings.sys.wsize:floor((frames-settings.sys.start+1)/settings.sys.wsize)*settings.sys.wsize
            ...
            tic
                [c1,c2]=sofiCumulantsSNRseq3(substack,1,[],[],settings.sys.orders,[],cellfun(@(x)x(:,:,[1:count-1,count+1:end]),...
                    sofi_c,'UniformOutput',0),nss,count,settings.sys.block);
            toc
...
%         sofi = sofi1;
        ...
        for io=settings.sys.orders
            for ii = 1:nss
               ...
               psf = fspecial('gaussian',io*[3 3],1);
%                temp = cellfun(@(x)sum(x(:,:,1:ii),3),sofi_c,'UniformOutput',0);
               temp = sum(deconvlucy(abs(sofi_c{io}(:,:,1:ii)),psf).^(1/io),3);%./ii;
               ...
               sofi2{io,ii}=sofi2{io,ii}/frame;
                    ...
                    sofi2{io,ii} = blkproc(sofi2{io,ii},blockSize,@(x)sum(x(:)));
                    end
