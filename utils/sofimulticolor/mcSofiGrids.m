%grid=mcSofiGrids(orders,dims,all)
%-------------------------------
%
...
%
function grid=mcSofiGrids(orders,dims,all)
...
if order==1
    ...
    if dims > 2
        pixels=nchoosek(0:7+order-2,order-1);       % pixel combinations
        pixels=[pixels(:,1) diff(pixels,1,2)-1];
        pixels=uint8([zeros(size(pixels,1),1) cumsum(pixels,2)]);
        pixels=cat(3,bitand(pixels,1), ...
            bitand(bitshift(pixels,-1),1), ...     % pixel coordinates
            bitshift(pixels,-2));
    else
    ...
    for iio=1:chend
        if mod(order,2)
            ...
            pixels((iio-1)*(order^2)+1,:,1)=[-1 zeros(1,order-2) 1];
        else
            pixels((iio-1)*(order^2)+1,:,1)=[-1*ones(1,order/2) ones(1,order/2)];
        end
    end
