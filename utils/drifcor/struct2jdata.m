...
if(~isempty(strcmp('x0x5F_ArrayType_',fn)) && ~isempty(strcmp('x0x5F_ArrayData_',fn)))
    newdata=cell(len,1);
    for j=1:len
        ndata=cast(data(j).x0x5F_ArrayData_,data(j).x0x5F_ArrayType_);
        iscpx=0;
        if(~isempty(strcmp('x0x5F_ArrayIsComplex_',fn)))
            if(data(j).x0x5F_ArrayIsComplex_)
                iscpx=1;
            end
        end
        if(~isempty(strcmp('x0x5F_ArrayIsSparse_',fn)))
            if(data(j).x0x5F_ArrayIsSparse_)
                if(~isempty(strcmp('x0x5F_ArraySize_',fn)))
        ...
        elseif(~isempty(strcmp('x0x5F_ArraySize_',fn)))
