...
%
function [sofi,grid,bias,snrs,vars]=sofiCumulantsSNR(stack,first,frames,region,orders,gpu)
file=ischar(stack);
if file
   tif=imfinfo(stack);
...
%       for p=orders
%       sofi{p}=medfilt2(sofi{p},[2 2]);
%       end
      
      for p=orders
         sofi1{p}=sofi1{p} + sofi{p};
         sofi2{p}=sofi2{p} + sofi{p}.^2;
      end
      fig=statusbar(frame/frames,fig);
      if isempty(fig)
         break;
