function [sofi,sofi_c,settings,sofi_lin,sofi_dec,stats,results] = SOFI2D_process(settings) 
% 2D SOFI processing 
...
% Tomas Lukes, tomas.lukes@epfl.ch
    ...
    elseif strcmp(settings.io.fext,'.dat') == 1 || strcmp(settings.io.fext,'.raw') == 1 
        ...
        id = 1;
        for i=1:nframes
            data_temp = fread(fid,[settings.io.W,settings.io.H],...
                'uint16',settings.io.byteorder);
            stack(:,:,id) = data_temp;
            id = id+1;
        end
        settings.sys.frames = nframes;
        settings.io.imageFile = char([settings.io.imagePath, ...
            filesep,settings.io.imageName_11]);
        settings.io.imageName_11 = char(settings.io.imageName_11);
        fclose('all');
    end
    ...
    if settings.io.dcor == 1 
        ...
        elseif strcmp(settings.dcor.type,'SOFI')
            ...
            for ii = 1:ii_max
                ...
                if ii==ii_max
                    drifts(((ii-1)*fnumber)+1:end, :) = repmat(...
                        [drift_x, drift_y],settings.sys.frames - (ii_max-1)*fnumber,1);
                else
                    drifts(((ii-1)*fnumber)+1:((ii-1)*fnumber)+fnumber, :) = repmat(...
                        [drift_x, drift_y],fnumber,1);
                end
            end
    ...
    if settings.augLag.deconv
        ...
        h2 = fspecial('Gaussian',[sy,sx],settings.augLag.FWHM*sqrt(n)/2/sqrt(2*log(2)));
            ...
            for k = 1:Nw
                ...
                tmp(tmp<0)=0;
                scale = max(tmp(:));
                sofi_dec{n}(:,:,k) = scale*deconvAugLag3D(tmp/max(tmp(:)),...
                    fftn(h2,[sy,sx]),settings.augLag);
            end
        end
    end
