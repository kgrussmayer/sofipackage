...
%
function grid=sofiGridsT(orders,all)
...
if order==1
    ...
    pixels=nchoosek(0:3+order-2,order-1);
    pixels=[pixels(:,1) diff(pixels,1,2)-1];
    pixels=uint8([zeros(size(pixels,1),1) cumsum(pixels,2)]);
    pixels=cat(3,bitand(pixels,1),bitshift(pixels,-1));
    ...
    [center,x]=unique(center(y,2:-1:1),'first','rows');
    pixels=double(pixels(y(x),:,:));
    %
    % Modify first pixel combination to eliminate the single-pixel-combination
    %
    if mod(order,2)
        pixels(1,:,1)=[-1 zeros(1,order-2) 1];
    else
        pixels(1,:,1)=[-1*ones(1,order/2) ones(1,order/2)];
    end
    
    %     pixels3=nchoosek(0:order-1,order-1);
    %     pixels3=[pixels3(:,1) diff(pixels3,1,2)-1];
    %     pixels3=uint8([zeros(size(pixels3,1),1) cumsum(pixels3,2)]);
    %     pixels3=reshape(repmat(pixels3,[1 size(pixels,1) 1]).',order,[]).';
    %     pixels=repmat(pixels,[order 1 1]);
    %
    %     pixels(:,:,1)=pixels(:,:,1)+1;
    %     pixels(:,:,2)=pixels(:,:,2)+1;
    %     pixels(:,:,end+1)=pixels3;
    pixels3=nchoosek(0:order-1,order-1);
    pixels3=[pixels3(:,1) diff(pixels3,1,2)-1];
    pixels3=uint8([zeros(size(pixels3,1),1) cumsum(pixels3,2)]);
    %     pixels3=pixels3(2:end,:);
    pixels3=reshape(repmat(pixels3,[1 size(pixels,1) 1]).',order,[]).';
    %     pixels=repmat(pixels,[order-1 1 1]);
    pixels=repmat(pixels,[order 1 1]);
