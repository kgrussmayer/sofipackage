...
function varargout = frctoresolutionMy(frc_in,sz,sector)
frc_in1 = frc_in;
...
nr = double(radSum4(ones(sz),sector))';
...
% 
if TB_curve
   p = pwd; % hack to avoid the function shadowing by smooth from dip_image
...
% disp(find(frc_in<1/7,1));
% isects  = frc_in(find((frc_in)<1/7,1));
% figure, plot(frc_in1);hold on;plot(frc_in,'r');
