...
%
function [sofi1,sofi2,sofi3]=sofiCumulantsSNRseq2(stack,first,frames,region,orders,gpu,sofiseq,nss,count,blockx)
file=ischar(stack);
if file
   tif=imfinfo(stack);
...
%       h = ones(blockSize);
      if blockx > 0
          for p=orders
              ...
              sofi{p} = blkproc(sofi{p},blockSize,@(x)sum(x(:)));
          end
      end
      
      for ii = count: nss 
        sofiseqsum = cellfun(@(x) sum(x(:,:,1:ii-1),3),sofiseq,'UniformOutput',0);
%         sofiseqsum2 = cellfun(@(x) mean(x(:,:,1:ii),3),sofi_c,'UniformOutput',0);
        for p=orders
         
         if blockx > 0
            blockSize = repmat(ceil((blockx*104/4)/(104/p)),1,2); %add ceil !
            temp = sofi{p} + blkproc(sofiseqsum{p},blockSize,@(x)sum(x(:))); %add sofi images from other subsequences
         else         
         temp = sofi{p} + sofiseqsum{p};
         end
