...
%
function [sofi,grid]=sofiCumulants3D(stacks,first,frames,region,orders,gpu)
[X,Y,Z,F]=size(stacks);
try
   ...
   grid=sofiGrids(orders,3);           % 3D partitions and pixels
   ...
   if gpu
      ...
      terms=parallel.gpu.GPUArray.zeros(X,Y,Z,T);
