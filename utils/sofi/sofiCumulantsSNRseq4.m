...
%
function [sofi1,sofi2]=sofiCumulantsSNRseq3(stack,first,frames,region,orders,gpu,sofiseq,nss,count,blockx)
file=ischar(stack);
if file
   tif=imfinfo(stack);
...
%       h = ones(blockSize);
      if blockx > 0
          for p=orders
              blockSize = repmat(ceil((blockx*104/4)/(104/p)),1,2);
...
%         sofiseqsum2 = cellfun(@(x) mean(x(:,:,1:ii),3),sofi_c,'UniformOutput',0);
        for p=orders
            psf = fspecial('gaussian',p*[3 3],1);
            if ii > 1
         sofiseqsum = cellfun(@(x) sum((deconvlucy(abs(x(:,:,1:ii-1)),psf)).^(1/p),3),sofiseq(p),'UniformOutput',0);
            else
                sofiseqsum(1) = {0};
            end
                
         if blockx > 0
            blockSize = repmat(ceil((blockx*104/4)/(104/p)),1,2); %add ceil !
            temp = sofi{p} + blkproc(sofiseqsum{1},blockSize,@(x)sum(x(:))); %add sofi images from other subsequences
         else 
             
         temp = deconvlucy(sofi{p},psf).^(1/p) + sofiseqsum{1};
