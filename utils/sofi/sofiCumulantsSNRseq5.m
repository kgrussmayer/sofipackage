...
%
function [sofi1,sofi2]=sofiCumulantsSNRseq5(stack,first,frames,region,orders,gpu,sofiseq,nss,count,blockx,gammas)
file=ischar(stack);
if file
   tif=imfinfo(stack);
...
%        for p = orders
%             blockSize = blockx*p;
% %             blockSize = ceil((blockx*104/4)/(104/p));
% %             sofiseq{p} = binStack(abs(sofiseq{p}).^gammas(p),blockSize);
%             sofiseq{p} = binStack(abs(sofiseq{p}),blockSize);
%        end
%     else
%       for p=orders
% %       sofiseq{p} =abs(sofiseq{p}).^gammas(p);
%       sofiseq{p} =abs(sofiseq{p});
%       end
%     end
...
%       sofiseq = cellfun(@(x)sum(x,3),sofiseq,'UniformOutput',0);
%       sofi=sofiAllFlatten(sofi,orders);
...
%                 blockSize = blockx*p;
% %               blockSize = ceil((blockx*104/4)/(104/p));
% %               sofi{p} = blkproc(abs(sofi{p}),blockSize,@(x)sum(x(:)));
% %                sofi{p} = binStack(abs(sofi{p}).^gammas(p), blockSize);
% %                 sofi{p} = binStack(abs(sofi{p}), blockSize);
%                 sofi{p} = abs(sofi{p});
%           end
%       else
%           for p=orders
% %           sofi{p} = abs(sofi{p}).^gammas(p);
%           sofi{p} = abs(sofi{p});
%           end
%       end
      
      for ii = count: nss 
%         sofiseqsum = cellfun(@(x) sum(abs(x(:,:,1:ii-1)),3),sofiseq,'UniformOutput',0);
%         sofiseqsum2 = cellfun(@(x) mean(x(:,:,1:ii),3),sofi_c,'UniformOutput',0);
        for p=orders
        
         
         blockSize = blockx*p;
%          sofiseqsum = cellfun(@(x) sum((abs(x(:,:,1:ii-1))).^(1/p),3),sofiseq(p),'UniformOutput',0);
%          temp = sofi{p} + sofiseqsum{1}; %add sofi images from other subsequences
       sofiseqsum = cellfun(@(x) sum(binStack(abs(x(:,:,1:ii-1)), blockSize),3),sofiseq(p),'UniformOutput',0); 
%          sofiseqsum = cellfun(@(x) sum(binStack(abs(x(:,:,1:ii-1)), blockSize).^(gammas(p)),3),sofiseq(p),'UniformOutput',0); 
%         sofiseqsum = cellfun(@(x) sum((abs(x(:,:,1:ii-1))),3),sofiseq(p),'UniformOutput',0);    
%          sofiseqsum = cellfun(@(x) sum(abs(x(:,:,1:ii-1)).^(gammas(p)),3),sofiseq(p),'UniformOutput',0); 
         
%         temp = binStack(abs(sofi{p}) + sofiseqsum{1}, blockSize).^gammas(p);
%           temp = binStack(abs(sofi{p}).^gammas(p) + sofiseqsum{1}, blockSize);
          
%         temp = binStack(abs(sofi{p}), blockSize).^gammas(p) + sofiseqsum{1};
           temp = (binStack(abs(sofi{p}), blockSize) + sofiseqsum{1}).^gammas(p);
%          temp = binStack(abs(sofi{p}) + sofiseqsum{1}, blockSize).^gammas(p);
        
%          temp = temp./max(temp(:));
%          bcg1 = median(temp(:));
%          temp = imadjust(temp,[0.8*bcg1 1],[0 1]);
         
%                       bcg1 = median(temp(:));
%                     temp = imadjust(temp,[0.8*bcg1 1],[0 1]);
%                       temp(temp<0.8*bcg1)=0;
                      
%          sofiseqsum = cellfun(@(x) sum((abs(x(:,:,1:ii-1))),3),sofiseq(p),'UniformOutput',0);    
%          temp = sofi{p} + sofiseqsum{1};
         
%              min(temp(:))
%              min(sofi{p}(:))
        
%          temp = temp./(ii); %mean sofi image from ii subsequences
% if p > 1
%           temp(temp<0.05*max(temp(:)))= 0;    
% end
%                count = 1;
%                for bb = [1 2 4 8 9 10 12]
%                    blockx=bb;
                   

%                    temp = blockproc(temp,blockSize,@(block_struct)sum(block_struct.data(:)));
%          temp = conv2(temp,h,'same'); 
%                end
         sofi1{p,ii}= sofi1{p,ii} + temp;
%          sofi2{p,ii}(:,:,frame)= temp;
         sofi2{p,ii}= sofi2{p,ii} + temp.^2;
...
% end
% bias=sofi;                             % cumulants offsets
% vars=sofi;                             % cumulants variances
% snrs=sofi;                             % signal-to-noise ratios
% for p=orders
%    sofi1{p}=sofi1{p}/frame;
%    sofi2{p}=sofi2{p}/frame;
%    bias{p}=(frame-1)*(sofi1{p} - sofi{p});
%    vars{p}=(frame-1)*(sofi2{p} - sofi1{p}.^2);
%    snrs{p}=abs(sofi{p})./sqrt(vars{p});
% end
